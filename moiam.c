...
#define NDCON 20
  ...
  for (i = 0; i < nstruct; ++i)
    { /* read in parameters */
      ...
      iatm = rdinp (luin, NDATM, &noatm, nx, wt, dist, theta, phi, istau,
                    xyztag);
      if (iatm == 0)
        {
      ...
      for (iatm = 0; iatm < n3atm; ++iatm)
        {
          ...
          if ((iatm % 3) == 2 && istau[ii] != C0)
          ...
          else
            {
              ...
              fc[nstruct] = 0.;
              ftran (NFT, pdbl, ND3ATM, fc,
                     -1); /* synthesize dist, theta, or phi */
            }
          ++pdbl;
        }
      pdbl = xcoord;
      ...
      for (iatm = 0; iatm < n3atm; ++iatm)
        {
          ...
          ftran (NFT, &dcoord[iatm], ND3ATM, fc,
                 1); /* synthesize derivative */
          ++pdbl;
        }
    }
...
static int
rdinp (FILE *luin, int ndatm, int *natm, int *nx, double *wtx, double *dist,
       double *theta, double *phi, char *istau, char **xyztag)
{
#define BLNK (' ')
#define NCARD 9
  static struct swt
...
static int
coords (FILE *lu, int noatm, double *coord, double *wt, int *nx, double *dist,
...
static int
gcalc (int natm, double *wt, double *coord, double *dcoord, double *gval,
       double *cval)
{
...
static double
determ (double *a, int ndd, int nr, int nc)
  ...
  for (itr = 0; itr < 100; ++itr)
    {
      ...
      if (pivot != 0)
        { /* rotate about z to make Cz constant */
          rho -= r;
          r *= pi2 / NFT;
...
static int
pamsolv (int natm, double *x, double *gval, double *tval)
{
...
static int
micvt (double *cval, double *gval, double vmuz, double vmuy, int flg)
{
...
static double
xinterp (int n, double *tauval, double *xtrans)
{
  double ang, ang1, tt;
  int i, k, ii;
  tt = 0.;

...
#define NZPAR 11
  ...
  static int rqpair[] = { 15, 11, 16, 12, 17, 13, 18, 14, 19, 14, 23, 20,
                          23, 21, 23, 22, 24, 20, 24, 21, 24, 22, 25, 20,
                          25, 21, 25, 22, 26, 20, 26, 21, 26, 22, 0,  0 };
  double *w, *ww, *pt, *pd;
  ...
  if (iblk == 0)
    {
      ...
      for (i = 0; rqpair[i] != 0; ++i)
        { /* add other required w values */
          ii = rqpair[i];
          jj = rqpair[++i];
...
int
setopt (FILE *luin, int *iqnfmt, int *itd, int *ndbcd, char *namfil)
{ /* SET OPTIONS */
  char card[82];
  double val[2];
...
int
setblk (FILE *lu, const int npar, bcd_t *idpar, const double *par, int *nblkpf,
        int *negy)
{
  int nsiz;
  double ai;

...
#define NFILE 2
  ...
  do
    { /*  begin master loop */
      ...
      while (iqfx > iqfmx)
        { /*   purge data for old F */
          while ((now = head[0]) != NULL)
            { /*  more data for F */
              k = 0;
              palow = now->a;
              pblow = now->b;
      ...
      if (frqc < big * strtot)
        ...
        { /* sum contributions to a and b and c*/
          iflg = iqfu - iqfmx;
...
static AB_STR *
lposn (char *iqnu, int idx, int nn)
{
  AB_STR *now, *last;
  int i;
...
static int
starks (double *a, double *b, double *c, int iqfx, int iqdel, int ifhalf)
{
  int idgn;
  double f, fsq, aa, den;

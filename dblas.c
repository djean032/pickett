...
CBLAS_INDEX cblas_idamax(const int n, const double *dx,
                         const int incX) { /*  FINDS THE INDEX OF ELEMENT HAVING
                                              MAX. ABSOLUTE VALUE. */
  double dmax, dtmp;
  int i, iret, incx;
  if (n <= 1)
...
double cblas_dasum(const int N, const double *dx,
                   const int incX) { /* TAKES THE SUM OF THE ABSOLUTE VALUES. */
  double dtmp, dret;
  int n, incx;
  n = N;
  if (n > 0) {
...
void cblas_daxpy(
    const int N, const double da, const double *dx, const int incX, double *dy,
    const int incY) { /* CONSTANT TIMES A VECTOR PLUS A VECTOR. LOOP UNROLLED */
...
void cblas_dcopy(const int N, const double *dx, const int incX, double *dy,
                 const int incY) { /* COPIES A VECTOR, X, TO A VECTOR, Y. */
  double da;
  int n, incx, incy;
  n = N;
  if (n > 0) {
...
double cblas_ddot(
    const int N, const double *dx, const int incX, const double *dy,
    const int incY) { /*  FORMS THE DOT PRODUCT OF TWO VECTORS. LOOP UNROLLED */
  double dret, *dx0, *dy0;
  int n, incx, incy;
  n = N;
  if (n <= 0)
...
void cblas_drot(const int N, double *dx, const int incX, double *dy,
                ...
                const double s) { /* APPLIES A PLANE ROTATION. */
  double dtmpx, dtmpy;
  int n, incx, incy;
  n = N;
  if (n > 0) {
...
void cblas_dscal(
    const int N, const double da, double *dx,
    const int incX) { /* SCALES A VECTOR BY A CONSTANT. LOOP UNROLLED */
...
void cblas_dswap(const int N, double *dx, const int incX, double *dy,
                 const int incY) { /* INTERCHANGES TWO VECTORS. */
  double dtmp;
  int n, incx, incy;
  n = N;
  if (n > 0) {

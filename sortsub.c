  ...
  do
    {
      ...
      if (nline < mxline)
        ...
        { /* MXLINE lines read without EOF after first pass */
          rewind (fscr);
          ftmp = fscr;
          fscr = fout;
          fout = ftmp;
        ...
        { /* MXLINE lines read without EOF on first pass */
          fout = tmpfile ();
          fscr = tmpfile ();
        }
      scrflg = (init || fillbuf (fscr, buf0, 1, pbuf) != 0);
      k = 0;
      while (scrflg || k < nline)
        { /* merge buffer with scratch file */
          if (scrflg && k < nline)
            {
...
int
fillbuf (FILE *finp, char *tbuf, int nlim, char **pbuf)
{
  int i, iret, ich;
  iret = 0;
  for (;;)
...
int
bufcmp (const char *b1, const char *b2, const int *key)
{
...
int
memsort (int nline, const int *key, char **pbuf)
{ /* heapsort */
  int left, right, i, j;
  char *pcmp;
  left = nline >> 1;
...
int
memfind (int ibgn, int nbuf, const int *key, const char *buf0, char **pbuf)
{
...
int
getkey (int key[], char tbuf[], char wbuf[])
{
  int k, i, init, klast;
  char cmp;

...
#define LBLEN 10
  ...
  nqn = (iqnfmt % 10) << 1;
  sprintf(card, "LINES REQUESTED=%5d NUMBER OF PARAMETERS=%3d", limlin, npar);
  fputs(card, lufit);
  fputs(card, stdout);
  ...
  do {
    ...
    do { /*   form least squares matrix, LOOP over lines */
      ...
      if (iblnd > 0) {
        ...
        fprintf(lufit, fmt_xbgn, line, aqnum, xfrq, afrq, adif, xerr, cerr);
        fputc('\n', lufit);
        lblnd = line + 1;
      } else {
        lblnd = line;
        do { /* UNTIL all elements of blend are printed */
          if (frqdat(lblnd, &iblnd, &xfrq, &xwt, &xerr, qnum)) {
            ...
            if (fabs(dif) > bigd)
              dif = (dif > 0.) ? bigd : -bigd;
            qnfmt(nqn, qnum, aqnum);
            fprintf(lufit, fmt_xbgn, lblnd, aqnum, xfrq, frq, dif, xerr, cerr);
    ...
    xsqt = sqrt(xsqt);
    ++itr;
    sprintf(card, " END OF ITERATION %2d OLD, NEW RMS ERROR=%15.5f %15.5f\n",
            itr, xsqt, xsq);
    fputs(card, lufit);
    fputs(card, stdout);
  ...
  sprintf(card, "%4d %4d %4d %4d %14.4E %14.4E %14.4E %12.10f\n", npar, limlin,
          nitr, nxpar, marqp, xerrmx, parfac, fqfacq);
  fputs(card, lupar);
  fputs(card, luvar);
...
int jelim(double *t, double *vec, int ndm,
...
int lineix(FILE *lu, int flg, int nline, int nblkpf,
           int iqnfmt) { /*   get lines from input  and stores them */

...
#define NRHOK 5
  ...
  for (ipar = 0; ipar < npar; ++ipar) { /*   print out parameters */
    ...
    fprintf(luout, "%4d %8ld %3d %3d %3d %3d %3d%c %15.5E %15.5E %s\n", ipar,
            idtyp, ixv, pxpecp->nval, pxpecp->mval, kdel, parp->offset, cval,
            val, parp->parerr, parp->labl);
    if (parp->offset != 0)
      parp->offset = nxpec;
    pxpecp = parp->ptxpec;
    if (pxpecp != NULL) {
  ...
  for (ipar = 0; ipar < npar; ipar = iend + 1) {
    ...
    if (idtyp < 0) {
      ...
      nnpar += wrpar(lu, &didtmp, 1, isdip, kflg0, txpec, 1, card, mv, drhok);
    }
    ksymtau = parp->ksym << 1;
    idtst = -idtyp;
    if (idtyp == 800099L)
    ...
    for (ivx = 0; ivx < nvtot2; ivx += nsig) {
      for (ivxp = 0; ivxp <= ivx; ivxp += nsig) {
          ...
          for (ivp = ivxp; ivp <= nvp; ++ivp) {
            ...
            if (isdip > 0) {
              ...
              if (nvfac == 100)
                didtmp = -9999.;
              nnpar +=
                  wrpar(lu, &didtmp, 1, 0, kflgm, xfc, 1, pbgn, nvfac2, drhok);
              didtmp = 0;
            }
            wrpar(luout, &didtmp, 1, isdip, kflg, txpec, nft, pbgn, nft, drhok);
            if (iv == ivp)
              pxfc[0] -= vref;
            if (ktest > 0 && nft2 == nft && dnrm2(nft, pxfc, 1) > tiny) {
              ...
              pbgn[0] = '/';
              pbgn[1] = ':';
              nnpar += wrpar(luout, &didtmp, 1, isdip, kflg, egy, ktest, pbgn,
                             ktest, 1.);
              pbgn[0] = ' ';
              pbgn[1] = '/';
            }
        ...
        nnpar += wrpar(lu, didtmpv, nvs, isdip, kflgm, xfc, nprn, pbgn, -nvfac2,
                       drhok);
        pxfc1 = NULL;
        pxfc = NULL;
        kflgm = -1;
...
int iamdiag(FILE *lu, int nvib, SXPEC **pxpecv, SEIG *eig, double *egy,
            int ioff) {
  SXPEC *pxpec0;
  ...
  for (iv = ioff; iv < iend; iv += nsym) { /* loop over states */
    ...
    if (init == 0) {
      ...
      egy0 = val;
      if (lu != NULL)
        fprintf(lu, "  v,    rho*K, vmix, pmix, energy relative to %15.6f\n",
                egy0 / c);
    }
    val -= egy0;
    if (lu != NULL)
      fprintf(lu, "%3d %9.6f %2d %9.6f %15.6f\n", iv, eig->rhok[ixvib], kmix,
              pmix, val / c);
    egy[iv] = val;
    icol = iv * nbasis;
...
int expec(int ityp, int indx, SXPEC **pxpecv, int nvib, SEIG *eigm, SEIG *eigp,
          int noff, int *isym) {
...
int getparv(int nv, int mv, int iv, int nvib2, SIPAR *parp, SXPEC **pxpecv) {
  static int n = 0;
...
int ideqv(long *pidtmp, double *pfac) {
#define IDIV 10000
  static struct {
    double fac;
    int ityp, kd;
  } cmpv[] = {
      {-2.0, 4, 2}, {0.5, 21, 2}, {-1.0, 40, 1}, {1.0, 60, 1}, {-1.0, 61, 1},
      {1.0, 41, 1}, {0.5, 22, 2}, {-2.0, 12, 2}, {1.0, 0, 0},
  };
  long idtmp;
  int kdret, i, ii, key;
...
int ftanal(int iflg, int nft, double *txpec, double *xfc) {
  int k, kk, nft2;
...
int ksign(int kdel, int nft, double *txpec, double *xfc) {
  double dif, val0;
  int k, kp, km, iret;
  iret = nft - kdel;
  if ((kdel & 1) != 0) {
...
void seig_alloc(SEIG *eigv, const int nvec, const size_t nfill, int *pmorgwk,
                const int nvib) {
  SEIG *eig;
  int *pmorg;
  int k;
...
int wrpar(FILE *lu, double *id, int nid, int isdip, int kflg, double *t, int nt,

...
int iamdiag (/*@null@*/ FILE *lu, int nvib, SXPEC **pxpecv,
             /*@out@*/ SEIG *eig, /*@out@*/ double *egy, int ioff);
int expec (int ityp, int indx, PSXPEC *pxpecv, int nvib, SEIG *eig, SEIG *eigp,
           int nxpec, int *isym);
int getparv (int nv, int mv, int iv, int nvib2, /*@out@*/ SIPAR *parp,
             PSXPEC *pxpecv);
int ideqv (long *idtmp, /*@out@*/ double *fac);
int init2top (FILE *lu, int nvib2, int nsym, int nvib, int nvtot, int nsig,
              short *kmap, S2TOP *vmap,
...
void spar_alloc (/*@out@*/ SIPAR *parwk, /*@out@*/ PSIPAR *par,
                 const int nvec);
...
#define NRHOK 5
  ...
  for (k = 0; k < nline2; ++k, ivv += ND2TOP)
    {
      if (fgetstr (card, 82, luin) < 1)
        break;
      dcopy (ND2TOP, &zero, 0, dval, 1);
      if (pcard (card, dval, ND2TOP, NULL) < 1)
        continue;
  ...
  nvtop = init2top (luout, nvib2, nsym, nvib, nvtot2, nsig, kmap, vmap, sigma2,
                    ivtop);
  free (kmap);
  ndtop = 0;
  ndmtop = 0;
  ...
  pxpecv
      = (SXPEC **)mallocq (nl); /* allocate vector of expectation pointers */
  nl = (size_t)(ntyp + 1) * sizeof (SXPEC);
  ...
  for (ivp = ivvp = 0; ivp < nvtot; ++ivp, ivvp += nbasis)
    {
              ...
              for (kk = 0; kk <= 1; ++kk)
                {
                  ...
                  vref = eigvec[nl];
                  if (fabs (vref) > fabs (vmax))
                    vmax = vref;
                  mv = 1 - (eigp->morg[ivv] << 1) / nsym;
                  eigvec = eigvec1;
                }
            }
          nneg = 0;
      ...
      if (ODD (isym[ivp]))
        { /* correct for translation symmetry */
          hsym = TRUE;
          eig = &eigv[0];
          for (k = 0; k < neig; ++k)
            {
                  ...
                  for (isigx = 0; isigx < nsym; ++isigx)
                    {
                      ...
                      pmorg[isig]
                          = sigma[isigx] + nsym * (int)floor (val + 0.5);
  ...
  if (luint != NULL)
    {
      if (fgetstr (title, 82, luint) > 0 && fgetstr (card, 82, luint) > 0)
        {
          fclose (luint);
          ...
          for (iv = 0; iv < nvib; ++iv)
            {
              ...
              nnpar += wrpar (lubak, &didtmp, 1, 0, kflg0, txpec, 1, pstr, 0,
                              drhok);
              ++ivv;
            }
        }
  ...
  if (nvib2 > 0)
    {
      ...
      nvtotsq = ndmtop * ndmtop;
      nxpec = nvtotsq * nft;
    }
  else
    {
      vmapsym = (int *)mallocq (sizeof (int));
      vmapsym[0] = 0;
    }
  ...
  for (ipar = 0; ipar < npar; ipar = iend + 1)
    {
      ...
      if (idtyp < 0)
      ...
      else
        {
          ...
          nnpar += wrpar (lu, &didtmp, 1, isdip, kflg0, txpec, 1, card, mv,
                          drhok);
        }
      ksymtau = parp->ksym << 1;
      idtst = -idtyp;
              ...
              if (sigma2[iv] >= 0 && iv > 0)
                {
                  ...
                  nnpar += wrpar (lu, &didtmp, 1, 0, kflg0, txpec, 1, pbgn, 0,
                                  drhok);
                }
              for (kk = 1; kk <= kmax; ++kk)
                {
          ...
          for (ivxp = 0; ivxp <= ivx; ivxp += nsig)
            {
              if (nvib2 > 0)
                {
                  isymtau = ksymtau ^ isym[vmap[ivx].ix1]
                            ^ isym[vmap[ivxp].ix1] ^ isym[vmap[ivx].ix2]
                            ^ isym[vmap[ivxp].ix2];
                }
              else
                {
                  ...
                  for (ivp = ivxp; ivp <= nvp; ++ivp)
                    {
                      ...
                      for (k = ipar; k <= iend; ++k)
                        {
                          ...
                          if (nvib2 > 0)
                            {
                              nl = (size_t)ndmtop * vmap[iv].inew
                                   + vmap[ivp].inew;
                              nl += nxpec * (k - ipar);
                              daxpy (nft, val, &xpec2top[nl], nvtotsq, txpec,
                                     1);
                            }
                          else
                            {
                      ...
                      if (isdip > 0)
                        ...
                        { /* idtyp == 99 */
                          ...
                          if (nvfac == 100)
                            didtmp = -9999.;
                          nnpar += wrpar (lu, &didtmp, 1, 0, kflgm, xfc, 1,
                                          pbgn, nvfac2, drhok);
                          didtmp = 0;
                        }
                      ...
                      if (ktest > 0 && nft2 == nft
                          && dnrm2 (nft, pxfc, 1) > tiny)
                        {
                          ...
                          pbgn[0] = '/';
                          pbgn[1] = ':';
                          nnpar += wrpar (luout, &didtmp, 1, isdip, kflg, egy,
                                          ktest, pbgn, ktest, 1.);
                          pbgn[0] = ' ';
                          pbgn[1] = '/';
                        }
              ...
              nnpar += wrpar (lu, didtmpv, nvs, isdip, kflgm, xfc, nprn, pbgn,
                              -nvfac2, drhok);
              pxfc1 = NULL;
              pxfc = NULL;
              kflgm = -1;
...
int
iamdiag (FILE *lu, int nvib, SXPEC **pxpecv, SEIG *eig, double *egy, int ioff)
{
  SXPEC *pxpec0;
...
int
expec (int ityp, int indx, SXPEC **pxpecv, int nvib, SEIG *eigm, SEIG *eigp,
       int noff, int *isym)
{
  ...
  for (iv2s = ibgn2; iv2s <= iend2; ++iv2s)
    {
      ...
      ivv0 = iv2s * nvtot;
      for (iv1s = ibgn1; iv1s <= nlim; ++iv1s)
        { /* iv1s <= iv2s */
          iv1d = (int)isym[iv1s] >> 2;
          if (iv1d == 0)
            iv1z = iv1s;
          isig1 = iv1s - iv1z;
          if (nvcmp == 0 && isig1 != isig2)
            continue;
          eigbra = eigp;
          eigket = eigm;
          iv1 = iv1s;
...
int
getparv (int nv, int mv, int iv, int nvib2, SIPAR *parp, SXPEC **pxpecv)
{
  static int n = 0;
...
#define IDIV 10000
  static struct
  {
    ...
    { -2.0, 4, 2 }, { 0.5, 21, 2 },  { -1.0, 40, 1 },
    { 1.0, 60, 1 }, { -1.0, 61, 1 }, { 1.0, 41, 1 },
    { 0.5, 22, 2 }, { -2.0, 12, 2 }, { 1.0, 0, 0 },
  };
  long idtmp;
  int kdret, i, ii, key;
...
int
ftanal (int iflg, int nft, double *txpec, double *xfc)
{
  int k, kk, nft2;
...
int
ksign (int kdel, int nft, double *txpec, double *xfc)
{
  double dif, val0;
  int k, kp, km, iret;
  iret = nft - kdel;
...
void
ftfix (int nft2, int nft, double *xfc)
{
...
void
sxpec_alloc (PSXPEC *pxpecv, const int nvec)
{
  SXPEC *pxpec;
  int i;
  pxpec = pxpecv[0];
...
void
spar_alloc (SIPAR *pswk, PSIPAR *par, const int nvec)
{
  SIPAR *pspar;
  int i;
  par[0] = pswk;
...
void
seig_alloc (SEIG *eigv, const int nvec, const size_t nfill, int *pmorgwk,
            const int nvib)
{
  SEIG *eig;
  int *pmorg;
...
int
init2top (FILE *lu, int nvib2, int nsym, int nvib, int nvtot, int nsig,
          short *kmap, S2TOP *vmap, int *sigma2, int *ivtop)
{
  S2TOP *pmap, *pmapp;
  ...
  for (k = 0; k < nvtot; ++k)
    {
      ...
      fprintf (lu, "%3d: %2d %3d %3d, %2d %2d%2d, %3d %3d\n", k, isig, isig1,
               isig2, i, iv1, iv2, sigma2[k], pmap->inew);
    }
  return nvtop;
} /* init2top */

int
get2top (SIPAR *ppar, /*@out@*/ double *xpec2, int nvtot, int nft, int nsym,
         int *vmapsym, int *ivtop, int nvtop)
{
  ...
  noff = ndtot * nft;
  iret = 0;
  isunit1 = pxpec1->isunit;
  isunit2 = pxpec2->isunit;
  nv1 = pxpec1->nval;
  nv2 = pxpec2->nval;
  /* load operators */
  ioff = 0;
  for (iv = 0; iv < ndm2; ++iv)
    {
      ...
      for (ivp = 0; ivp < ndm2; ++ivp)
        {
          ivv = ivp / nsymsq;
          isigp = ivp - ivv * nsymsq;
          isig1p = isigp / nsym;
          isig2p = isigp - isig1p * nsym;
          if (nv1 == 0 && isig1p != isig1)
            continue;
          if (nv2 == 0 && isig2p != isig2)
            continue;
          ivv = ivv << 1;
          iv1p = isig1p + nsym * ivtop[ivv];
          iv2p = isig2p + nsym * ivtop[ivv + 1] + nvib;
          if (isunit1 == 1 && iv1p != iv1)
            continue;
          if (isunit2 == 1 && iv2p != iv2)
            continue;
          ix1 = iv1 + nvtot * iv1p;
          ix2 = iv2 + nvtot * iv2p;
          va1 = dxpec1[ix1];
      ...
      for (ivp = 0; ivp <= iv; ++ivp)
        {
          ...
          vb1 = xpec2[ix1 + ioff];
          vb2 = xpec2[ix2 + ioff];
          if (fabs (va1 - va2) > 1.e-10)
            break;
          if (fabs (vb1 - vb2) > 1.e-10)
            break;
        }
      if (ivp <= iv)
        break;
...
int
wrpar (FILE *lu, double *id, int nid, int isdip, int kflg, double *t, int nt,
       char *labl, int nfmt, double drhok)
{
  /* id = parameter id */
          ...
          if (fabs (tval) > cmp)
            {
              ...
              fprintf (lu, fmt1, fid, ires, tval, labl);
              ++nret;
            }
          else
            {
          ...
          if (fabs (tval) > cmp)
            {
              ...
              fprintf (lu, fmt1, fids, ires, tval, labl);
              ++nret;
            }
          else
            {

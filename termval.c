...
#define NDLINE 128
  ...
  head_line.freqx = 0.;
  head_line.errfq = tiny;
  head_line.seq = 0;
  ndline = sizeof (LINE);
  norg = nline = 0;
  /*  read and sort experimental lines */
  while (getlin (lulin, nqn, idqn, iqnv, &xfrq, &xerr, &xwt, tmpstr, NDLINE)
         > 0)
    {
      if (xerr > 0.)
        {
      ...
      pline->next = head_line.next;
      head_line.next = pline;
      pline->plower = pegylo;
      pline->pupper = pegyup;
      pline->freqx = xfrq;
      pline->errfq = xerr;
  ...
  while (pegyup != &head_egy)
    { /* sort energies according to iorg */
      k = 0;
  ...
  for (pegyup = head_egy.prev; pegyup != &head_egy; pegyup = pegyup->prev)
    {
      ...
      last_line = NULL;
      k = 0;
      for (pline = bgn_line; pline != NULL; last_line = pline, pline = tmpline)
        {
          tmpline = pline->next;
      ...
      if (nfreq > 1)
        {
          ...
          if (tmpline != &head_line)
            {
              last_line = tmpline;
              crit_line = tmpline->next;
              last_line->next = crit_line->next;
              crit_line->next = head_line.next;
              head_line.next = crit_line;
            }
        }
  ...
  do
    {
      ...
      pqn = pegy->iqn;
      if (oldorg != norg)
        { /* find lowest energy in stack */
          pegyorg = pegy;
          oldorg = norg;
          egyorg = pegy->egyx;
          for (pegyup = pegy->next; pegyup->iorg == norg;
               pegyup = pegyup->next)
            {
              if (pegyup == &head_egy)
                break;
...
static EGYS *
lposn (short *ikey, const int nkey, int *iret)
{
  EGYS *now, *plast;
  short *iqnx;
  int k, icmp, iord;

...
#define FALSE 0
int
readqn (const char *qnstr, short *iqn, const int n)
...
int
pcard (const char *card, double *val, const int nval, const int *fmtlen)
  ...
  while (kval < nval)
    {
      ...
      if (ich >= '0' && ich <= '9')
      ...
      else if (ich == '.' && pwrflg == 0)
        { /* first decimal point */
          if (ndec < 0)
            ndec = 0;
          pwrflg = 1;
        }
      else if (ich == '-' && pwrflg == -1)
        { /* leading - in exponent field */
          negpwr = TRUE;
          ndec = 0;
          pwrflg = -2;
        }
        ...
        { /* character is not a number or decimal point, E+, E- */
          if (ndec >= 0)
            { /* save results from number decoding */
              if (pwrflg < 0)
                { /* integer follows 'E' */
                  pwrflg = 0;
                  if (negpwr)
           ...
           */
          if (ich == '.')
            { /* decimal afer exponent field, assume new */
              ndec = 0;
              pwrflg = 1;
            }
          else if (ich == '-')
            { /* leading - in mantissa field */
              ndec = 0;
              neg = TRUE;
            }
          else if (ich == '+')

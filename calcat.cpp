...
#define NDVEC 10
  ...
  if (ntemp == NTEMP)
    { /* sort temperatures in decreasing order */
      temp[NTEMP - 1] = tmq;
      for (k = NTEMP - 2; k >= 0; --k)
        {
          if (temp[k] > tmq)
  ...
  for (iblk = inblk; iblk <= lblk; ++iblk)
    {
      ...
      if (prfrq)
        {
          fprintf (luout,
                   " ENERGIES FOR BLOCK NUMBER %3d, INDEX-DEGEN-ENERGY-",
                   iblk);
      ...
      for (i = 0; i < isiz; ++i, teigp += isiz)
        {
          ...
          if (prfrq)
            {
              fprintf (luout, "%6d %4d %17.6f %17.6f %10.6f", iblk, i + 1, egx,
                       err, pmix[i]);
              if (globfmt != 0)
          ...
          if (pregy)
            {
              fprintf (luegy, "%6d %4d %17.6f %17.6f %10.6f %4d:", iblk, i + 1,
                       egx, err, pmix[i], idgn);
              if (globfmt != 0)
              ...
              for (j = 0; j < jmax; ++j)
                {
                  ...
                  if (first)
                    {
                      fputs (
                          " FREQUENCY-EST.ERROR.-LINE.STR. DIP**2-LGSTR.-ITD,",
                          luout);
                  ...
                  if (prfrq)
                    {
                      fprintf (luout, "%13.4f %8.4f %12.5E %8.4f %2d", frq,
                               err, strr, strlg, itd);
  ...
  printf ("INITIAL Q = %14.4f, NEW Q IS RELATIVE TO MIN.EGY.= %14.4f\n", qrot,
          egymin);
  fprintf (luout,
           "INITIAL Q = %14.4f, NEW Q IS RELATIVE TO MIN.EGY.= %14.4f\n", qrot,
           egymin);
...
int
qnfmt (short *iqu, int nqn, char *sqn)
{ /* subroutine to do quick conversion of quantum integers
     to characters */
...
int
simt (int isiz, int jsiz, double *s, double *t, double *u, double *wk)
{
  int i, j;
  double *sij, *si, *tc, *uc;
...
SBLK *
ibufof (const int ipos, const unsigned int ndel, SBLK *blk)
{
  /* Initialized data */
  static FILE *scratch;
          ...
          if (ldisk == maxrec && lret == len)
            {
              ...
              if (n > 0)
                {
                  len *= n;
                  lret = (long)fwrite (pvec, sizeof (double), (size_t)len,
                                       scratch);
                }
              maxrec += lsizb;
            }
...
SBLK *
sblk_alloc (const int nstruct, const unsigned mxdem)
{
  SBLK *pret, *pblk;
  int k;

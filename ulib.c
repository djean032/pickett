...
int
ordblk (const int ndm, const int n, short *iqnsep, double *t, double *e,
        short *isblk, double *p, short *ipasgn)
{
  ...
  if ((int)isblk[1] >= n && nsep == 1)
    ...
    { /* find largest projection for each eigenvector */
      if (nsep > 1)
        { /* isolate subsets defined by hdiag */
          for (eig = 0; eig < n; ++eig)
            {
              ipasgn[eig] = 0;
              p[eig] = -1.;
          ...
          for (isep = 1; isep <= nsep; ++isep)
            {
              iend0 = iend;
              for (iend = iend0; iend < n; ++iend)
                { /* find number in subset */
                  if (iqnsep[iend] != isep)
                    break;
                }
      ...
      tbase = t;
      for (eig = 0; eig < n; ++eig)
        { /* first pass at finding projection */
          pbgn = 1.;
          pcmp = -1;
...
void
etswap (const int ndm, const int nsize, const int ix1, const int ix2,
...
int
hdiag (const int ndm, const int n, double *z, double *d, double *e,
       short *iqsep)
{
  ...
  if (n == 1)
    return 1;
  if (isgn < 0)
    { /* CHANGE SIGN TO MAKE HOUSEHOLDER REFLECTION INTO ROTATION */
      z[0] = -z[0];
      z[1] = -z[1];
    }
...
int
triag (const int ndm, const int n, const int nz, double *z, double *d,
       double *e)
{
  /* Local variables */
  int i, k, m, nm, m0, itr;
...
int
getpar (FILE *lu, FILE *luout, int *nfit, int *npar, bcd_t *idpar, double *par,
        double *erpar, char *plbl, int plblen)
...
int
getvar (FILE *lu, const int nfit, double *var, bcd_t *idpar, double *erpar,
        const int iflg)
{ /*  SUBROUTINE TO READ VARIANCE INTO PACKED UPPER
     TRIANGULAR */
#define NCARD 130
  ...
  n = 0;
  if (iflg == 0)
    { /* number of requested parameters == actual */
      n = nfit;
      k = nv = 0;
      pvar = var;
      for (j = 0; j < n; ++j)
...
int
putvar (FILE *lu, const int nfit, double *var, double *erpar)
{ /*  SUBROUTINE TO WRITE VARIANCE FROM PACKED UPPER
     TRIANGULAR */
...
double
calerr (const int nfit, const double *var, const double *derv)
{
  double err, sum;
  int n;
...
int
getlin (FILE *lu, const int nqn, short *idqn, short *iqn, double *xfreq,
        double *xerr, double *xwt, char *card, const int ncard)
...
int
getbcd (const char *line, bcd_t *ivbcd, int nbcd)
{
...
int
putbcd (char *line, int nlen, const bcd_t *ivbcd)
{
  int k, first, i, kk, n;
  k = (int)(ivbcd[0] & 0x7f);
  n = k + k;

...
#define NDLINE 128
  ...
  if (fgetstr (line, NDLINE, luin) <= 0)
    exit (0);
  nn = NDQN + 2;
  for (k = 0; k < nn; ++k)
    dvec[k] = 0.;
  if (pcard (line, dvec, nn, NULL) < 2)
    exit (0);
  nqn = (int)dvec[0];
  jmax = (int)dvec[1];
  if (nqn > NDQN || nqn <= 0)
  ...
  while (fgetstr (line, NDLINE, luin) > 0)
    {
      ...
      for (k = 0; k < nn; ++k)
        dvec[k] = 0.;
      dvec[1] = (double)jmax;
      if (pcard (line, dvec, nn, NULL) == 0)
        continue;
      now = (PRULE)mallocq (sizeof (RULE));
      ++nrule;
      now->next = head;
      head = now;
      now->jbgn = (short)dvec[0];
      now->jend = (short)dvec[1];
      now->knt = (short)nrule;
      now->iqn1[0] = 0;
      now->iqn2[0] = 0;
      pqn1 = now->iqn1;
...
int
fixqn (const int nqn, short *iqn, int *key, const DRULE head)
{
  RULE *now;
  short *pqn;
  int k;

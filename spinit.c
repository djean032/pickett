...
void
getzitot (double *zfac, int ltot, int ii, const int *lscomv, const int *iscomv,
          const int *jscomv, int alpha, int neqi)
{ /* get reduced matrix elements of nuclear spins */
  ...
  itot = (int)((unsigned int)iscomv[nm2] >> 1);
  isym = iscomv[0];
  jtot = (int)((unsigned int)jscomv[nm2] >> 1);
  jsym = jscomv[0];
  itdif = itot - jtot;
  isgn = 0;
  if (itdif < 0)
...
void
setzitot (int lv1, int lv2, int lvt, int ii, int neqi)
      ...
      for (itot = 0; itot < itmax; ++itot)
        {
          ...
          pitot->ioff[k] = -1;
          jtot = itot - itdif;
          if (jtot < 0 || itot + jtot + ifhalf < lvt)
            continue;
          ipnow = itbase + itot;
          jpnow = itbase + jtot;
          ni = ipnow->n;
              ...
              for (i = 0; i < ni; ++i)
                {
                  qni = &ipnow->qn[nm1 * i];
                  for (k = 0; k < nm2; ++k)
                    { /* check intermediate spins */
                      iscomv[k] = qni[k];
                      jdif = qni[k] - qnj[k];
                      if (jdif < 0)
                        jdif = -jdif;
                      if (jdif > lscomv[k]
                          || (int)(qni[k] + qnj[k]) < lscomv[k])
                        break;
                    }
                  if (k == nm2)
                    {
...
ITMIX *
get_itmix (const int ii, const int neqi)
{
  ITMIX *pret;
  EITMIX *pitmix;
  ...
  for (ktot = 0; ktot < nq; ++ktot)
    {
      ...
      if (kcmp == 0)
      ...
      else
        {
          ...
          for (k = 0; k < neven; ++k)
            {
              pitmix->qsym[k] = (short)0;
              pitmix->offset[k] = (short)0;
            }
          for (k = neven; k < nitot; ++k)
            {
              pitmix->qsym[k] = (short)2;
              pitmix->offset[k] = (short)0;
            }
          ++pitmix;
          qn0 = qn;
...
double
ctrans (int i1, int i2, int ii, short *qn, int nqsym, short *swk)
{

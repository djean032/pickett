...
#define NDLINE 130
  ...
  for (;;)
    {
      if (getlin (lulin, nqnlin, iqn1, iqn2, &xfrq, &xerr, &xwt, tmpstr,
                  NDLINE)
          < 0)
        {
          fclose (lulin);
  ...
  while (noeof)
    {
      ...
      if (nline2 >= minb)
        ...
        { /* set NOEOF = false for end of file */
          noeof = FALSE;
        }
...
void
prline (FILE *lu, int nqn, short *pqn, double frq, double err, double st,
        char *label)
{
  int k;
  for (k = 0; k < nqn; ++k)
...
int
lposn (const short *ikey, const int nkey, const int *k12, PEXPT *head,
       PDEXPT *ppexpt)
{
  EXPT *now, *last;
  short *iqnx;
...
void
dolink (PEXPT *head, PDEXPT pexpt, const double xfrq, const double xerr,
        const short *iqn)
{
...
void
unlink (PEXPT *head, EXPT *pexpt, int *jmin, int *jmax)
{
  EXPT *now, *last;
  int jx, jj;
  jx = (int)(pexpt->iqn[0]);

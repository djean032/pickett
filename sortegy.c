...
#define NFILE 6
#define NDLINE 128
  static const char *cext[NFILE]
      = { "egy", "lgy", "inp", "lin", "opt", "log" };
  enum efile
  {
    eegy,
    elgy,
  ...
  while (fgetstr (tmpstr, NDLINE, luegy) > 0)
    {
      dtmp[0] = -1.;
      if (pcard (tmpstr, dtmp, nqn + 6, NULL) < 1)
        break;
      iblk = (int)dtmp[0];
      egyc = dtmp[2];
      for (k = 0; k < nqn; ++k)
  ...
  for (;;)
    {
      if (getlin (lulgy, nqn, iqnq, iqn2, &xfrq, &xerr, &xwt, tmpstr, NDLINE)
          < 0)
        {
          break;
        }
  ...
  while (pegy1 != NULL)
    {
      ...
      if (pegy1 == NULL)
        break;
      if (pegy1->block != plast->block)
        continue;
      pegy2 = pegy1->cross;
      plast2 = plast->cross;
      if (pegy2 == NULL || plast2 == NULL)
        continue;
      ...
      if (fabs (dif2) > tiny && fabs (dif) > tiny)
      ...
      else
        {
          if (qncmp (nqn, pegy1->iqn, pegy2->iqn) == 0
              && qncmp (nqn, plast->iqn, plast2->iqn) == 0)
            continue;
        }
      pegy1->cross = plast2;
      plast->cross = pegy2;
  ...
  if (luinp != NULL)
    {
      ...
      for (;;)
        {
          if (getlin (luinp, nqn, iqnq, iqn2, &xfrq, &xerr, &xwt, tmpstr,
                      NDLINE)
              < 0)
            {
              break;
...
int
qncmp (const int nqn, const short *qn1, const short *qn2)
{
  int j, iret;
  iret = -1;
  for (j = 0; j < nqn; ++j)
...
PDEGY
qnfind (EGY *head, const short *iqn, const int nqn)
{
  PDEGY pegy;
  if (*iqn < 0)
    return NULL;
...
void
qncopy (const int nqn, const short *qn1, short *qn2)
{
  int j;
  qn2[0] = qn1[0];
  for (j = 1; j < nqn; ++j)
...
void
egymove (PDEGY plast, PDEGY pdest)
{
  PEGY psrc;
  if (plast != NULL)
    {

...
#define LBLEN 10
  ...
  do
    {
                  ...
                  if (frqdat (lblnd, &iblnd, &xfrq, &xwt, &xerr, qnum))
                    {
                      ...
                      if (fabs (dif) > bigd)
                        dif = (dif > 0.) ? bigd : -bigd;
                      qnfmt (nqn, qnum, aqnum);
                      fprintf (lufit, fmt_xbgn, lblnd, aqnum, xfrq, frq, dif,
                               xerr, cerr);
      ...
      dlsq (fit, ndfit, nfit, nfit, 1, erpar,
            iperm); /*  invert lower triangle */
  ...
  sprintf (card, "%4d %4d %4d %4d %14.4E %14.4E %14.4E %12.10f\n", npar,
           limlin, nitr, nxpar, marqp, xerrmx, parfac, fqfacq);
  fputs (card, lupar);
  fputs (card, luvar);
...
int
jelim (double *t, double *vec, int ndm, int n)
...
int
qnfmt (int nqn, short *qnum, char *aqnum)
{
  /* Local variables */
  int i;

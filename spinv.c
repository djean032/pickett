...
int dpmake(const int nsize, double *dp, const double *t, const int n,
           const double *wk, const short *ix, const short *jx,
           const int isunit);
...
int setwt(SVIB *pvinfo, const int ivib, const int iax, const int iwtpl,
...
int dircos(const int *xbra, const int *xket, const int ld, const int kd,
           const int ncmax, /*@out@*/ double *direl,
           /*@out@*/ short *ibra, /*@out@*/ short *iket, const int ifup,
           const int loff, int mask,
           /*@out@*/ int *isunit);
      ...
      for (jxx = 0; jxx <= ixx; ++jxx) { /*  set up sub-block quanta */
        ...
        for (spar_now = spar_head[ivmin]; TRUE; spar_now = spar_now->next) {
          ...
          if (TEST(idflags, MNSQ) && nd == 0) {
            ...
            if (ncos < 0) {
              isunit = kavg;
              ncos = dircos(ixcom, jxcom, ld, kd, ndmx, wk, idx, jdx, ijd, kl,
                            mkd, &isunit);
              if (ncos <= 0) {
                if (ncos == 0)
                  continue; /* get next parameter */
              ...
              if (neuler != 0) { /*Euler operator */
                ncos = specop(neuler, &newblk, &nsqj, &ikq, kbgni, kbgnj, ni,
                              nj, ncos, wk, idx, jdx, par[ipar]);
                if (ncos == 0)
                  continue;
              }
              if (ifc != 0)
                specfc(ifc, ivbase, jvbase, kd, kbgni, kbgnj, ncos, wk, idx,
                       jdx);
              if (sznz != 0)
                sznzop(ni, nj, kbgni, kbgnj, iscom, jscom, ncos, wk, idx, jdx);
...
int dclr(const int n1, const int n2, double *vec,
         const int ix) { /*  clear a N1*N2 block */
...
unsigned int
blksym(const int *ixcom,
       const int *jxcom) { /* get block symmetry from state symmetry */
...
int bestk(const int ndm, const int nsize, short *iqnsep, short *ibkptr,
          short *itau, short *iswap, double *t, double *egy, double *pmix,
          double *wk) {
#define MAXNK 4
  double ele;
...
double
rmatrx(const int ld, const int lv, const int *ixcom,
       const int *jxcom) { /* find  N corrections to reduced matrix elements */
...
int dpmake(
    const int nsize, double *dp, const double *t, const int n, const double *wk,
    const short *idx, const short *jdx,
    const int isunit) { /*  find derivative contribution from sub-diagonal */
  const double *pt;
  double ele, tz;
  int i, k, iz, jz;
...
int tensor(double *zval, const int iscom[], const int jscom[],
  ...
  for (i = 0; i < npair; ++i) {
    ...
    if (alpha >= 0 && ix == itptr) {
      ix = itsym + nspin + 1;
      getzitot(&z, lls, iscom[ix], &lscom[ix], &iscom[itsym], &jscom[itsym],
               alpha, glob.nitot);
      i = nspin - 1;
    }
    llj = lscom[i];
...
int setint(FILE *lu, BOOL *ifdiag, int *nsav, const int ndip, bcd_t *idip,
           int *isimag) { /*   subroutine to initialize intensity data */
  static char *bad_msg[] = {
      "\n",
  ...
  *nsav = 1;
    ...
    if (bad_dip != 0) {
      if (bad_dip > 0) {
        ...
        if (bad_dip * sizeof(char *) >= sizeof(bad_msg))
          bad_dip = 0;
        fprintf(lu, " WARNING: dipole %3d %s has no matrix elements, %s",
  ...
  if (nmin > 0) {
    for (i = 0; i < ndip; ++i) {
      ...
      /* set dipole to zero */
      putbcd(sbcd, NSBCD, &idip[ibcd]);
      fprintf(lu, " WARNING: dipole %3d %s has bad phase and is ignored\n",
              (i + 1), sbcd);
      dipinfo[i].fac = 0.;
    ...
    for (jxx = 0; jxx < nblkj; ++jxx) {
      ...
      for (i = 0, ibcd = 0; i < ndip; ++i, ibcd += nbcd) {
        ...
        do {
          ...
          if (isunit > nx)
            continue;
          ncos = dircos(ixcom, jxcom, ld, kd, ndmx, wk, idx, jdx, ifup, kl, mkd,
                        &isunit);
          if (ncos == 0)
            continue;
          if (ncos < 0) {
...
int setblk(FILE *lu, const int npar, bcd_t *idpar, const double *par,
  ...
  double ai;
  size_t nl;
  int nodd, isym, jsym, ixzt, i, k, kl, lt, n, kdiag, nvibm, neven, nsize,
      alpha;
    ...
    if (EVEN(isym)) { /* even */
      ...
      if (i != ii || pvinfo->wt[0] != pvinfo->wt[2] ||
        ...
        fprintf(lu, " %4d %4d %4d %4d %4d %6d    B\n", iv, knnmin,
                pvinfo->knmax, pvinfo->wt[2], pvinfo->wt[1], ii);
      }
    }
    ++pvinfo;
  }
...
int pasort(FILE *lu, const int npar, bcd_t *idpar, const double *par) {
  ...
  while (ipar >= -1) {
    ...
    do { /* repeat until parameter subtypes exhausted */
      ...
      idpars(spar_now, &ikq, &neuler, &lt, &ld, &kd, &ins, &si1, &si2, &sznz,
             &ifc, &alpha, &ldel, &kavg);
      iiv1 = pvib1->spt;
      iiv2 = pvib2->spt;
      if (si1 > 0 &&
    ...
    if (i > ipar) {
      if (notused > 0) {
        ...
        if (notused * sizeof(char *) >= sizeof(strej))
          notused = 0;
        fprintf(lu, " WARNING: parameter %6d %s has no matrix elements: %s",
...
int idpari(bcd_t *idval, int itp, SPAR *pspar) {
  ...
  if (exval != 0) {
    ...
    ibtmp = (bcd_t)exval;
    exval = exval >> 3;
    if (idval[1] == (bcd_t)0 && idval[2] == (bcd_t)0 && idval[3] == (bcd_t)0 &&
        idval[4] == ibtmp) {
      /* energy Euler term */
      if (idval[0] == (bcd_t)0)
...
BOOL testwt(SVIB *pvib1, SVIB *pvib2, int isym,
            int alpha) { /* return true if all weight sets do not match */
...
int getqn(const int iblk, const int indx, const int maxqn, short *iqn,
  ...
  if (iblk == cgetv[0].cblk) {
    ...
    cgetq = &cgetv[1];
    ioff = glob.maxblk;
    nsblk = getqq(iblk, &cgetq->cff, cgetq->cwt, &ibkptr[ioff], &ikmin[ioff],
                  &ivs[ioff]);
    if (nsblk == 0) {
      *idgn = 0;
      return glob.nqn;
...
int dircos(
    const int *xbra, const int *xket, const int ld, const int kd,
    const int ncmax, double *direl, short *ibra, short *iket, const int ifup,
    const int loff, int mask,
    int *isunit) { /*  SUBROUTINE TO  CALCULATE DIRECTION COSINE ELEMENTS */

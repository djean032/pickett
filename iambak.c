...
#define NDVAL 8 /* NDVAL >= ND2TOP  and NDVAL >= NDPARI*/
  ...
  for (k = 0; k < nvib2; ++k) {
    ...
    if (pcard(buf, dval, ND2TOP, NULL) < 0)
      continue;
  ...
  /* read old par file */
  if (npar <= 0 || fgetstr(buf, NDBUF, luin2) < 0)
    exit(EXIT_FAILURE);
  pcard(buf, dval, 1, NULL);
  nxpar = (long)dval[0];
  nvib = 0;
  for (;;) { /* read option cards */
    if (fgetstr(buf, NDBUF, luin2) < 0)
      break;
    dval[0] = 0.;
    dval[7] = 1.;
    if (readopt(buf, dval, 8) <= 0)
      break;
    if (nvib == 0)
      nvib = (int)fabs(dval[0]);
    if (dval[7] > -0.5)
      break;
  }
  if (nvib >= 99)
  ...
  for (ixpar = 0; ixpar < nxpar; ++ixpar) {
    ...
    for (ipar = 1; ipar < npar; ++ipar) {
      ...
      del = dval[0] - parp->parval;
      parp->parval = dval[0];
      parp->parerrv = dval[1];
      k = 0;
      if (idtmp == 2000 && nv == 0 && mv == 1)
        k = 1;
      if (k == 0 && idtmp == 0 && nv > 0 && mv == 0)
        k = 1;
      if (k == 0 && idtmp == 0 && nv == 0 && mv == 2)
        k = 1;
      if (k != 0) {
        printf("%2d %3d %2d, new=%12.3f, change=%12.3f ", parp->v21, nv, mv,
               parp->parval, del);
        err = parp->parerrv;
        if (fabs(err) < 1.e+08)
  ...
  for (k = 0; k <= nxv; ++k) {
    ...
    if (ivv < 0)
      ivv = irho[9];
    if (ivv >= 0)
      par[ivv].iflg &= (MNEG | MDIP); /* clear MUSED */
    irho[k] = ivv;
    if (icval[k] < 0)
      icval[k] = icval[9];
    if (ifval[k] < 0)
      ifval[k] = ifval[9];
    if (icval[k] < 0 || ifval[k] < 0)
      irho[k] = -1;
  }
  /* update rho */
  for (k = 0; k <= nxv; ++k) {
  ...
  for (ipar = 1; ipar < npar; ++ipar) {
    ...
    fprintf(luout, "%12ld  %3d %2d %23.15e", idtmp, parp->nv, parp->mv,
            parp->parval);

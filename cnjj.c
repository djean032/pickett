...
double
c3jj (const int j1, const int j2, const int j3, const int m1, const int m2,
      const int m3)
...
int
deljj (const int jxa, const int jxb, const int jxc, double *val)
{
  int ia, ib, ic, isum;
...
{ /* 9J COEFFICIENT CALCULATION */
  ...
  lperm = 0;
  if (kmin >= 3)
    { /* EXCHANGE COLUMN SO JMIN IS IN FIRST COLUMN */
      if (kmin < 6)
        {
          kmin -= 3;
          jjp = &jj[3];
  ...
  if (kmin > 0)
    { /* EXCHANGE ROW SO JMIN IS IN FIRST ROW AND COLUMN */
      jjp = &jj[kmin];
      for (kk = 0; kk < 9; kk += 3)
        {
          j = jj[kk];

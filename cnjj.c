...
int cjjini(void) { /**** set up log factorials ****/
  int nfac;
  double v;
  v = 0.;
  fac[0] = 0.;
...
double c3jj(const int j1, const int j2, const int j3, const int m1,
            const int m2, const int m3) { /*    3J COEFFICIENT CALCULATION */
  double trm, val;
...
double
c6jj(const int j1, const int j2, const int j3, const int jj1, const int jj2,
     const int jj3) { /* 6J COEFFICIENT CALCULATION    USING INTEGERS = 2 J */
  double trm, val;
  ...
  k3 = k - k3;
  k4 = k - k4;
  val = exp(0.5 * val + fac[++k] - fac[la] - fac[lb] - fac[lc] - fac[k2] -
            fac[k3] - fac[k4]);
  if ((k & 1) == 0)
    val = -val;
  trm = val;
...
double c9jj(int jj[]) { /* 9J COEFFICIENT CALCULATION */
  double val;
  int *jjp;
  ...
  if (jmin == 0) { /* SPECIAL FOR  J=0 */
    ...
    for (kk = kmin; kk <= kmax; kk += 2) {
      val += (double)(kk + 1) * c6jj(jj[0], jj[1], jj[2], jj[5], jj[8], kk) *

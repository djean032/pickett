                      ...
                      if (ncos < 0)
                        {
                          ...
                          if (neuler != 0)
                            { /*Euler operator */
                              ncos = specop (neuler, &newblk, &nsqj, &ikq,
                                             kbgni, kbgnj, ni, nj, ncos, wk,
                                             idx, jdx, par[ipar]);
                              if (ncos == 0)
                                continue;
                            }
          ...
          else
            {
              ...
              switch (glob.idiag)
                ...
                case 5:
                  ...
                  if (glob.idiag == 4)
                    {
                      bestk (nsize, nsize, iqnsep, vbkptr, idx, jdx, t, egy,
                             pmix, wk);
...
int
ordham (const int nn, short *mask, double *egy, const short *isblk,
        short *iswap)
{
...
int
bestk (const int ndm, const int nsize, short *iqnsep, short *ibkptr,
              ...
              for (ii = is; ii < iend; ++ii)
                {
                  ...
                  if (k > kd)
                    break;
                  /* find basis with K == kd, nk = the number of basis with
                   * same K */
                  ++nk;
                } /* loop ii */
            }
  ...
  for (i = 0; i < npair; ++i)
    {
      ...
      if (alpha >= 0 && ix == itptr)
        {
          ix = itsym + nspin + 1;
          getzitot (&z, lls, iscom[ix], &lscom[ix], &iscom[itsym],
                    &jscom[itsym], alpha, glob.nitot);
          i = nspin - 1;
        }
      llj = lscom[i];
      jji = iscom[i];
              ...
              for (/* no init */; spar_now != NULL; spar_now = spar_now->next)
                {
                  ...
                  klp = idpars (spar_now, &ikqp, &neulerp, &ltp, &ldp, &kdp,
                                &insp, &si1p, &si2p, &sznzp, &ifcp, &alphap,
                                &ldelp, &kavgp);
                  klp &= MMASK;
      ...
      for (;;)
        {
          ...
          if (ODD (kk))
            { /* bad phase */
              ...
              putbcd (sbcd, NSBCD, &idpar[ibcd]);
              fprintf (lu,
                       " WARNING: parameter %6d %s is imaginary and will not "
                       "be used\n",
                       (ipar + 1), sbcd);
...
BOOL
testwt (SVIB *pvib1, SVIB *pvib2, int isym, int alpha)
...
dircos (const int *xbra, const int *xket, const int ld, const int kd,
        ...
        const int ifup, const int loff, int mask, int *isunit)
